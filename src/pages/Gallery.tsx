...
export function Gallery() {
  return (
    <div className="bg-nrn-cream pt-20">
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-16">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="font-display text-6xl md:text-8xl text-nrn-text mb-6 leading-none">
              Built To <em className="text-nrn-brick">Last.</em>
            </h1>
            <p className="text-nrn-muted text-xl max-w-2xl leading-relaxed">
              Every project tells a story of integrity and craftsmanship. No filters, just high-performance results across the Valley.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {/* Stats */}
      <div className="bg-nrn-warm border-b border-nrn-border">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-16 py-10 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={i} className="text-center">
              <div className="font-display text-3xl text-nrn-brick mb-1">{stat.v}</div>
              <div className="font-bold text-sm text-nrn-text">{stat.l}</div>
              <div className="text-xs text-nrn-muted mt-0.5">{stat.s}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {PHOTOS.map((photo, i) => (
              <motion.div key={photo.id} {...fadeUp} transition={{ delay: (i % 9) * 0.06 }}
                className="relative group cursor-zoom-in break-inside-avoid border border-nrn-border overflow-hidden bg-nrn-warm hover:shadow-lg transition-shadow">
                <img src={photo.src} alt={photo.title}
                  className="w-full min-h-[220px] object-cover transition-transform duration-700 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-nrn-navy/80 via-nrn-navy/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                  <span className="text-xs font-bold text-nrn-gold bg-nrn-navy/60 px-2 py-1 uppercase tracking-widest self-start mb-2 font-mono-label">
                    {photo.tag}
                  </span>
      ...
      {/* Case Study */}
      <section className="py-20 bg-white border-y border-nrn-border">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-16 grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="font-display text-5xl text-nrn-text mb-8 leading-tight">
              Anatomy of a<br /><em className="text-nrn-brick">Storm-Ready Install.</em>
            </h2>
            <p className="text-nrn-muted text-lg mb-10 leading-relaxed">
              Every NRN install follows a strict multi-layer protocol: cool-roof rated shingles, high-reflectivity underlayment, and industrial-grade ridge vents engineered for Arizona's extreme attic heat.
            </p>
            <ul className="space-y-3">
              {[
                ...
                <li key={i} className="flex items-center gap-3 text-sm font-semibold text-nrn-text">
                  <div className="w-2 h-2 bg-nrn-brick shrink-0 rounded-full" />
                  {feat}
                </li>
              ))}
            </ul>
          </div>
          <div className="border border-nrn-border overflow-hidden">
            <img src="/images/Untitled-design-16-scaled.png" alt="Storm repair site detail"
      ...
      {/* Closing CTA */}
      <section className="py-24 bg-nrn-warm border-b border-nrn-border text-center">
        <h3 className="font-display text-4xl text-nrn-text mb-4">See Your Home In Our Next Update?</h3>
        <p className="text-nrn-muted text-sm mb-10">Every job we finish goes in the portfolio — yours could be next.</p>
